...
from .crypto import isPassword, getDigest
...
class AddNewStat(MethodView):
    ...
    def post(self):
            ...
            print(data_dict)
            try:
                username = data_dict['User'][0]
                stepName = data_dict['Name'][0]
                Duration = data_dict['Duration'][0]
                priority = int(data_dict['priority'][0])
                status = int(data_dict['status'][0])

                document_exist = mongo_db_collection.find({'data.Name': stepName})

                if document_exist.count() == 0:
                    _id = mongo_db_collection.insert({"meta": "stepicStudioVideoObj"})
                else:
                    _id = document_exist.__getitem__(0)['_id']
